...
#define print_backtrace()                                                      \
	do {                                                                   \
		if (!(getenv("BACKTRACE") != NULL                              \
		      && !strcmp(getenv("BACKTRACE"), "true")))                \
			break;                                                 \
		void *addresses[10] = {0};                                     \
...
#define panic(fmt, ...)                                                        \
	do {                                                                   \
		fprintf(stderr, "[panic: %s@%s:%d] " fmt "\n", __func__,       \
...
typedef struct token_s {
	// The type of the token, this is a value from the enum above, it
	// represents how the union should be read - if at all
	token_type_t type;
	uint64_t span_s; // This is the start of the span where the token was
	                 // read

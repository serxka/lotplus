...
static const struct {
	...
	token_type_t type;
        ...
        {"ret", 3, T_RET},      {"module", 6, T_MODUL}, {"import", 6, T_IMPOR},
        {"export", 6, T_EXPOR}, {"struct", 6, T_STRUC}, {"enum", 4, T_ENUM},
        {"union", 5, T_UNION},  {"priv", 4, T_PRIV},    {"extern", 6, T_EXTRN},
        {"operator", 8, T_OP},
};
...
static inline char escaped_char(char final_c) {
	...
	if (c == '\\') {
		...
		case '\'':
			return '\'';
		default:
			panic("unknown escaped character (%c) at %ld:%ld", c,
			      lex_column(cursor), lex_linenum(cursor));
		}
	} else if (c == 0) {
		panic("character stream ended abruptly at %ld:%ld",
		      lex_column(cursor), lex_linenum(cursor));
	}
	++cursor;
...
static char char_scan(void) {
	char c = escaped_char('\'');
	if (c == 0)
		panic("zero length char at %ld:%ld", lex_column(cursor),
		      lex_linenum(cursor));
	if (current_src[cursor++] != '\'')
		panic("oversized char at %ld:%ld", lex_column(cursor),
		      lex_linenum(cursor));
	return c;
}
...
static void ident_scan(token_t *token) {
	...
	for (uint64_t i = 0; i < KEYWORD_TOKEN_LEN; ++i) {
		if (keyword_tokens[i].len != ident.len)
			continue;
		if (!strncmp(keyword_tokens[i].name, ident.d,
		             keyword_tokens[i].len)) {
...
token_t lex_next(void) {
	...
	if (current_src[cursor] == 0)
		return (token_t){
		        .type = T_EOF, .span_s = cursor, .span_e = cursor};
	...
	case '\'':
		...
		SET_TYPE(T_CHR);
	default: // If something else, it is either a keyword/identifer/number
		...
		if (ident_begin(c))
			...
			panic("unknown character '%c' (%d) at %ld:%ld", c,
			      (unsigned int)c, lex_column(cursor),
			      lex_linenum(cursor));
	}

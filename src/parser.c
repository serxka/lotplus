...
static bool expect(token_type_t type) {
	if (match(type))
		...
		panic("expected token %s at %ld:%ld, found token %s", token_debug_str[type], lex_column(token.span_s), lex_linenum(token.span_s), token_debug_str[token.type]);
}

// pretty self explanatory i reckon smh, git gud
// no, really, lower value == higher predence
// the left and right values are the associativity, (lower), (higher) is left-associative, guess what right is
// if change these make sure to add to the filter below as well
// TODO: fix these, i fucked them up, they are the wrong way
static const struct {
	uint8_t r;
} prefix_binding_pow[] = {
	[A_NEG] = {23},
	[A_PREINC] = {21}, [A_PREDEC] = {21},
};
...
node_t *parse_arguments(void) {
	...
	while (match(T_COMMA)) {
		...
		if (expr == NULL)
			panic("expected argument, found none: %ld:%ld", lex_column(token.span_s), lex_linenum(token.span_s));
		ast_push_child(args, expr);
	}
	return args;
}
...
node_t *expression_base(void) {
	if (token.type == T_IDENT || token.type == T_DOT) {
		...
		return ast_leaf(A_INTLIT, (union ast_val){.sint = last_token.sint});
	} else if (match(T_STR)) {
		return ast_leaf(A_STRLIT, (union ast_val){.string = last_token.string});
	}
	return NULL;
}
	...
	while (true) {
		...
		if (is_postfix_op(token.type)) {
			...
			if (op == A_IDX) {
				...
				expect(T_IDENT);
				node_t *rhs = ast_leaf(A_IDENT, (union ast_val){.identifier = last_token.string});
		...
		if (is_bin_op(token.type)) {
			...
			if (rhs == NULL)
				panic("unfinshed expression, missing right hand side on line: %ld:%ld", lex_column(token.span_s), lex_linenum(token.span_s));
			lhs = ast_binary(op, lhs, rhs);
			continue;
		}
		break;
